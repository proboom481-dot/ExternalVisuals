...
public class DamageNumbers extends Module {
    ...
    /*
     * Attack targets are kept for a short client-side window.
     * This makes damage numbers reliable even when the crosshair
     * moves away during the tick in which health changes.
     */
    private final Map<Entity, Integer> recentAttacks =
    ...
    public void markAttacked(Entity target) {
        ...
        recentAttacks.put(target, tickCounter + 8);
    }

    public boolean wasRecentlyAttacked(Entity target) {

        if (target == null) {
            return false;
        }

        Integer expiry = recentAttacks.get(target);
    ...
    public void onTick() {
        ...
        Iterator<Map.Entry<Entity, Integer>> attacks =
                recentAttacks.entrySet().iterator();

        while (attacks.hasNext()) {
            Map.Entry<Entity, Integer> attack = attacks.next();
            if (attack.getKey() == null
                    || attack.getValue() == null
                    || attack.getValue() < tickCounter) {
                attacks.remove();
            }
        }
    ...
    public ColorSetting getCriticalHitColorSetting() {
