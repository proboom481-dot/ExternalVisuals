...
public final class AimAssist extends Module {
    ...
    private final BooleanSetting visibleOnly =
            addSetting(new BooleanSetting(
                    "Visible Only",
                    true
            ));

    private final BooleanSetting horizontalOnly =
            addSetting(new BooleanSetting(
                    "Horizontal Only",
                    false
            ));
    ...
    private Entity findTarget() {
        ...
        double maxRange =
                Math.max(1.0, range.getValue());

        double maxFov =
                Math.max(1.0, fov.getValue());
        ...
        for (Entity entity : mc.world.getEntities()) {
            ...
            if (isPlayer && !players.isEnabled()) {
                continue;
            }

            if (!isPlayer && !mobs.isEnabled()) {
                continue;
            }
            ...
            if (visibleOnly.isEnabled()
                    && !mc.player.canSee(entity)) {
                continue;
            }

            float yawDifference =
                    Math.abs(getYawDifference(entity));

            if (yawDifference > maxFov / 2.0f) {
                continue;
            }
    ...
    public BooleanSetting getVisibleOnlySetting() {
        return visibleOnly;
    }

    public BooleanSetting getHorizontalOnlySetting() {
