...
public class TntTimer extends Module {
    ...
    private final BooleanSetting pulseDanger = addSetting(new BooleanSetting("Pulse Danger", true));
    private final BooleanSetting shadow = addSetting(new BooleanSetting("Shadow", true));
    private final BooleanSetting distance = addSetting(new BooleanSetting("Show Distance", false));
    private final BooleanSetting centered = addSetting(new BooleanSetting("Centered", true));
    private final SliderSetting pulseSpeed = addSetting(new SliderSetting("Pulse Speed", 6.0, 1.0, 12.0, 0.5));
    private final SliderSetting offsetY = addSetting(new SliderSetting("Offset Y", 0.0, -1.0, 2.0, 0.1));
    private final BooleanSetting compact = addSetting(new BooleanSetting("Compact", false));
    private final ColorSetting shadowColor = addSetting(new ColorSetting("Shadow Color", 0xAA000000));
    private final BooleanSetting showFuseBar = addSetting(new BooleanSetting("Fuse Bar", true));
    ...
    public String getText(TntEntity tnt) {
        ...
        boolean secondsEnabled = showSeconds.isEnabled();
        boolean ticksEnabled = showTicks.isEnabled();
        ...
        } else if ("BOTH".equalsIgnoreCase(selectedFormat)) {
            ticksEnabled = true;
            secondsEnabled = true;
        } else if ("SECONDS".equalsIgnoreCase(selectedFormat)) {
            secondsEnabled = true;
        }

        if (secondsEnabled && ticksEnabled) {
     ...
     */
    public float getOffsetY() { return offsetY.getValue().floatValue(); }
    public boolean isShadowEnabled() { return shadow.isEnabled(); }
    public int getShadowColor() { return shadowColor.getColor(); }
    public boolean isCentered() { return centered.isEnabled(); }
    public boolean isCompact() { return compact.isEnabled(); }
    public boolean isShowDistance() { return distance.isEnabled(); }
    public boolean isPulseDanger() { return pulseDanger.isEnabled(); }
    public float getPulseSpeed() { return pulseSpeed.getValue().floatValue(); }
    public boolean isFuseBarEnabled() { return showFuseBar.isEnabled(); }
