...
public final class ModuleSettingsPanel {
    ...
    private static final int[] COLOR_PALETTE = {
            0xFFFFFFFF, 0xFFFF4D67, 0xFFFF9E43, 0xFFFFE05A,
            0xFF55E86B, 0xFF42D9FF, 0xFF4D7CFF, 0xFF9B5CFF,
            0xFFE45CFF, 0xFF8B8F9B
    };
    ...
    public void render(
        ...
        maxScroll = getMaxScroll(settings);
        scrollOffset = Math.max(0.0, Math.min(maxScroll, scrollOffset));

        int settingY =
                y + 66 - (int) Math.round(scrollOffset);
        ...
        for (Setting<?> setting : settings) {
            ...
            boolean visible =
                    settingY + rowHeight >= contentTop
                            && settingY <= contentBottom;
        ...
        if (maxScroll > 0.0) {
            ...
            int thumbHeight = Math.max(24, (int) Math.round(
                    trackHeight * trackHeight / (double) Math.max(trackHeight, getContentHeight(settings))
            ));
            int thumbTravel = Math.max(0, trackHeight - thumbHeight);
            int thumbY = trackTop + (int) Math.round(
                    thumbTravel * (scrollOffset / maxScroll)
            );
    ...
    private void renderColorPicker(
        ...
        int bottom = pickerY + pickerSize + 66;
        ...
        for (int i = 0; i < COLOR_PALETTE.length; i++) {
            int swatchX = pickerX + 10 + i * 12;
            int swatchY = paletteY + 14;
            drawColorCircle(
                    matrices,
                    swatchX + 5,
                    swatchY + 5,
    ...
    public boolean mouseClicked(
        ...
        if (colorPickerOpen && activeColor != null) {
            ...
            if (button == 0) {
                ...
                int paletteY = rainbowY + 30;
                if (mouseY >= paletteY + 10 && mouseY <= paletteY + 28) {
                    int startX = pickerX + 10;
                    int index = (int) ((mouseX - startX) / 12.0);
                    if (index >= 0 && index < COLOR_PALETTE.length) {
                        int alpha = activeColor.getBaseColor() & 0xFF000000;
                        activeColor.setColor(alpha | (COLOR_PALETTE[index] & 0x00FFFFFF));
                        syncPickerFromColor(activeColor);
                        return true;
                    }
                }
        ...
        for (Setting<?> setting : settings) {
            ...
            int rowHeight = getSettingHeight(setting);
            if (settingY + rowHeight < y + 58 || settingY > y + height - 50) {
                settingY += rowHeight;
                continue;
            }
    ...
    public boolean mouseScrolled(
        ...
        if (mouseX < x || mouseX > x + width
                || mouseY < y + 52 || mouseY > y + height - 44) {
            return false;
        }

        maxScroll = getMaxScroll(module.getSettings());
        if (maxScroll <= 0.0) {
            return false;
        }

        double step = amount > 0.0 ? -34.0 : 34.0;
        scrollOffset = Math.max(0.0, Math.min(maxScroll, scrollOffset + step));
        return true;
    }
    ...
    private int getContentHeight(List<Setting<?>> settings) {
        int total = 0;
        if (settings != null) {
            for (Setting<?> setting : settings) {
                if (setting != null) {
    ...
    private double getMaxScroll(List<Setting<?>> settings) {
        int contentHeight = getContentHeight(settings);
        int visibleHeight = Math.max(1, height - 116);
        return Math.max(0.0, contentHeight - visibleHeight);
    }
