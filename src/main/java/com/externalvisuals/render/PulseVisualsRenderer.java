...
public final class PulseVisualsRenderer {
    ...
    private static void renderHud(MatrixStack matrices, float tickDelta) {
        ...
        try {
            ...
            if (pulse.isHitMarkerEnabled() && pulse.getHitMarkerAlpha() > 0.005f) {
                drawHitMarker(matrices, w / 2, h / 2, pulse);
            }
    ...
    private static void drawDynamicIsland(MatrixStack matrices, MinecraftClient mc, PulseVisuals pulse) {
        ...
        if (pulse.showIslandTarget() && mc.targetedEntity instanceof net.minecraft.entity.LivingEntity) {
            net.minecraft.entity.LivingEntity target = (net.minecraft.entity.LivingEntity) mc.targetedEntity;
            String name = target.getDisplayName().getString();
            if (name.length() > 18) name = name.substring(0, 18);
            parts.add(name);
            if (pulse.showIslandArmor()) {
                parts.add("ARM " + armorSummary(target));
            }
        }
    ...
    private static String armorSummary(net.minecraft.entity.LivingEntity target) {
        ...
        for (net.minecraft.item.ItemStack stack : target.getArmorItems()) {
            ...
            if (stack.isEmpty()) {
                ...
                result.append(Math.round(remaining * 100.0f / max));
            } else {
                result.append("100");
            }
        }
    ...
    private static void drawHitMarker(MatrixStack matrices, int cx, int cy, PulseVisuals pulse) {
        float alpha = pulse.getHitMarkerAlpha();
        int color = pulse.getHitMarkerColor();
        int a = Math.max(0, Math.min(255, Math.round(alpha * ((color >>> 24) & 255))));
        int c = (a << 24) | (color & 0x00FFFFFF);
        int size = pulse.getHitMarkerSize();
        int inner = Math.max(2, size / 3);
        drawThickLine(matrices, cx - size, cy - size, cx - inner, cy - inner, c, 2);
        drawThickLine(matrices, cx + size, cy - size, cx + inner, cy - inner, c, 2);
        drawThickLine(matrices, cx - size, cy + size, cx - inner, cy + inner, c, 2);
        drawThickLine(matrices, cx + size, cy + size, cx + inner, cy + inner, c, 2);
    }
