...
public final class ExtraHudRenderer {
    ...
    private static void renderTargetHud(
        ...
        if (hud.isArmorEnabled()) {
            ...
            details.append("Armor ").append(armorCount).append("/4");
        }
        ...
        int height = hud.isHealthBarEnabled() ? (details.length() > 0 ? 58 : 46) : (details.length() > 0 ? 46 : 34);
        ...
        try {
            ...
            if (hud.isBorderEnabled()) {
                DrawableHelper.fill(matrices, 0, 0, width, 2, accent);
            }
