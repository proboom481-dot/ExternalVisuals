...
public final class DamageNumbersRenderer {
    ...
    private static void render(
        ...
        for (DamageNumbers.DamageEntry entry
            ...
            /*
             * All three coordinates must be relative to the camera.
             * The old code subtracted camera X/Z but not Y, which made
             * damage numbers float at the wrong height when the camera
             * moved vertically (jumping, sneaking, slabs, etc.).
             *
             * Keep rendering dead entities here so the killing blow remains
             * visible until the damage entry expires.
             */
            ...
            boolean critical = damageNumbers.isCriticalColorEnabled()
                    && entry.getDamage() >= damageNumbers.getCriticalThreshold();
            if (critical) {
                baseColor = damageNumbers.getCriticalNumberColor();
            } else {
            ...
            String text = formatDamage(entry.getDamage(), damageNumbers.isCompact(), damageNumbers.isShowPlus());
            ...
            float scale = Math.max(0.0125f, 0.025f * damageNumbers.getSize());
            if (critical && damageNumbers.isCritScaleEnabled()) scale *= 1.18f;
            if (damageNumbers.isPulseEnabled()) {
                scale *= (float) (1.0 + 0.08 * Math.sin(System.currentTimeMillis() * 0.001 * damageNumbers.getPulseSpeed()));
            }
            ...
            try {
                ...
                if (damageNumbers.isOutlineEnabled()) {
                    int outlineColor = Math.max(0, Math.min(255, alpha)) << 24;
                    textRenderer.draw(text, textX - 1.0f, 0.0f, outlineColor, false,
                            matrices.peek().getModel(), consumers, true, 0, 15728880);
                    textRenderer.draw(text, textX + 1.0f, 0.0f, outlineColor, false,
                            matrices.peek().getModel(), consumers, true, 0, 15728880);
                    textRenderer.draw(text, textX, -1.0f, outlineColor, false,
                            matrices.peek().getModel(), consumers, true, 0, 15728880);
                    textRenderer.draw(text, textX, 1.0f, outlineColor, false,
                            matrices.peek().getModel(), consumers, true, 0, 15728880);
                }
    ...
    private static String formatDamage(float damage, boolean compact, boolean showPlus) {
        if (damage <= 0.0f) return "0";
        String value;
        if (compact || damage == (int) damage) value = Integer.toString(Math.round(damage));
        else value = String.format(Locale.US, "%.1f", damage);
